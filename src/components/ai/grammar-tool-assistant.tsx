...
export default function GrammarToolAssistant() {
  ...
  const handleGetExplanation = async () => {
    ...
    try {
        const prompt = `You are a friendly and helpful English grammar expert. Provide a clear, simple, and accurate explanation for the user's question with examples. Return the result as a valid JSON object with the key "explanation". User's question: "${question}"`;
        ...
        if (result.success && result.response) {
            try {
              ...
              // Assuming the JSON has a specific structure, e.g., { explanation: "..." }
              setExplanation(parsedResponse.explanation || result.response);
            } catch (jsonError) {
              // If parsing fails, it's likely plain text, so use it directly
