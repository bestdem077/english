...
interface ImprovedSentenceResponse {
...
export default function SentenceImprover() {
  ...
  const handleImproveSentence = async () => {
    ...
    try {
        const prompt = `You are an English language expert. Your task is to correct any grammatical errors and improve the user's sentence to make it sound more natural. Provide a brief explanation of the changes you made. Return the result as a valid JSON object with the keys "improved_sentence" and "explanation".\n\nUser's sentence: "${sentence}"`;
        ...
        if (result.success && result.response) {
            try {
                // Sanitize the response to remove markdown fences
                const cleanedResponse = result.response.replace(/```json|```/g, '').trim();
                const parsed: ImprovedSentenceResponse = JSON.parse(cleanedResponse);

                let formattedResponse = '';
                if (parsed.improved_sentence) {
                    formattedResponse += `Improved: ${parsed.improved_sentence}\n\n`;
                }
                if (parsed.explanation) {
                    formattedResponse += `Explanation: ${parsed.explanation}`;
                }
                
                setImprovedSentence(formattedResponse || result.response);

            } catch (jsonError) {
                // If parsing fails, it's likely plain text. Use it directly.
